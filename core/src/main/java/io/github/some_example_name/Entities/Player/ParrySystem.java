...
public boolean checkMissileParry(Contact contact, Fixture fixtureA, Fixture fixtureB) {
    if (!isParryActive) {
        Gdx.app.log("ParrySystem", "Parry não está ativo");
        return false;
    }
    ...
    if (isMeleeVsMissile) {
        ...
        if (!missile.isReflected() && missile.getOwner() != null) {
            ...
            Vector2 returnDirection = missile.getOwner().getPosition()
                    .cpy().sub(missile.getPosition()).nor(); // Usa a posição do míssil, não do jogador
            ...
            Gdx.app.log("ParrySystem", "Míssil não pode ser rebatido - já refletido: " + 
                missile.isReflected() + ", owner: " + missile.getOwner());
        }
    } else {
        Gdx.app.log("ParrySystem", "Não é colisão MELEE_ATTACK vs Missile");
    }

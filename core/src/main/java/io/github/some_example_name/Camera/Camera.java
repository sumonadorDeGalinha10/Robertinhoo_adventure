...
public class Camera {
    ...
    public void centerOnPlayer(Robertinhoo player, float offsetX, float offsetY) {
        ...
        // Converter posição do corpo para coordenadas de renderização (pixels)
        float playerScreenX = playerPos.x * MapRenderer.TILE_SIZE + offsetX;
        float playerScreenY = playerPos.y * MapRenderer.TILE_SIZE + offsetY;
        
        // Centralizar a câmera nas coordenadas de renderização
        camera.position.set(
            playerScreenX,
            playerScreenY,
            0
        ...
        camera.zoom = 3.0f; // Zoom mais equilibrado
        camera.update();
    }
